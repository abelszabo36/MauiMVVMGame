...
namespace MaciLaciMaui
{
    public class GameModel
    {
        ...
        #region Constructor
        public GameModel(DataAccess persistence)
        {
            this.datacess = persistence;
        }
        #endregion 
        ...
        public void NewGame(string level)
        {
            ...
            int[,] matrix = datacess.ReadFile(level); // palya beolvasasa
            timer = 0;
            collected = 0;
            needCollect = 0;
            int counter = 0;
        ...
        public void NewGame(int[,] level)
        {
            ...
            int[,] matrix = level; // palya beolvasasa
            timer = 0;
            collected = 0;
            needCollect = 0;
            int counter = 0;
        ...
        #region Movements
        public void HeroMove(int currentx, int currenty, int size, string direction) // a hos mozgasa
        {
            int x = currentx / size; // palyameret alapjan az aktualis pozicio kiszamitasa (ellentetesen mivel x y kordinatarendszer ellentetes a matrix beallitasaval)
            int y = currenty / size;
            if (table != null)
            {
                if (direction == "left") // balra lepes
                {
                    if (ObstacleCheck(x, y - 1)) // megnezi hogy bal oldalt van e akadaly
                    {
                        ...
                        OnMoving(currentx, currenty - size); // uj poziciot allitunk be a mozgas esemeny argumentumaba
                    }
                }

                if (direction == "right") // jobra lepes
                {
                ...
                if (direction == "down") // lefele lepes
                {
        ...
        public void PrintMatrix()
        {
            for (int i = 0; i < table?.TableSize; i++)
            {
                ...
                {
                    Debug.Write(table.GetField(i, j) + " ");
                }
                Debug.WriteLine("");
            }
        }
